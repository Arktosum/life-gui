...
export default function Finance() {
  ...
  let financeElements = financeItems.map((item) => {
    balance += item.amount * (item.mode == "SEND" ? -1 : 1) * (item.status == "PAID" ? 1 : 0);
    owed += item.amount * (item.mode == "SEND" ? -1 : 1) * (item.status == "PAID" ? 0 : 1);
    return (
      <FinanceItem
        key={item._id}
        item={item}
    ...
    <div className="h-[90vh] bg-black flex p-5">
      <div className="w-1/4 h-full bg-[#0B0B0B] rounded-xl">
        ...
        <div className="text-white text-center text-xl">
          Owed : ₹ 
          <span
            className={`${owed < 0 ? "text-red-600" : "text-green-600"}`}
          >
            {owed}
          </span>
        </div>
...
function FinanceItem({ item, setfinanceItems }) {
  ...
  async function updateStatus(){
    ...
    let updatedItem = await axios.post(`/api/finance/update`,{
        _id : item._id,
  ...
  return (
    <div
      className={`bg-[#141414] p-5 rounded-xl border-b-2 ${
    ...
    >
      ...
      <div className="flex justify-evenly items-center">
        ...
        <div className="flex gap-2">
          ...
          className="rounded-full w-5 h-5 hover:scale-125 duration-200 cursor-pointer">
            {item.status == "UNPAID" ? IncompleteIcon : completeIcon}
          </div>
        </div>
      </div>
    </div>
